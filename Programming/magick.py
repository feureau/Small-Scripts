...
def main():
    parser = argparse.ArgumentParser(description="Convert images using ImageMagick")
    parser.add_argument("patterns", nargs="*", help="Glob pattern(s) or file paths (default: all supported types), optionally followed by target format (e.g., 'png', 'webp')")
    parser.add_argument("-q", "--quality", type=int, default=DEFAULT_QUALITY, help="Output quality (1-100)")
    ...
    if args.format:
        ...
        is_format_specifier = (
            last_arg and
            len(last_arg) <= 5 and
            not any(c in last_arg for c in ['*', '?', '[', '/', '\\', '.']) and
            not os.path.exists(last_arg)
        )
        if is_format_specifier and len(args.patterns) >= 1:
            target_format = last_arg.lower()
            valid_patterns = [p for p in args.patterns[:-1] if p.strip()]
        else:
