...
def parse_item_id(url: str) -> str:
    ...
    raise ValueError("Could not parse Cambridge item id from URL.")


def fetch_manifest(session: requests.Session, item_id: str) -> dict:
    candidates = [
        f"https://cudl.lib.cam.ac.uk/iiif/{item_id}",
        f"https://cudl.lib.cam.ac.uk/iiif/{item_id}/manifest",
        f"https://cudl.lib.cam.ac.uk/v1/items/{item_id}",
    ]
...
def main():
    parser = argparse.ArgumentParser(
        description="Download all page images from a Cambridge CUDL item URL."
    )
    parser.add_argument("url", help="Example: https://cudl.lib.cam.ac.uk/view/MS-NN-00003-00074/1")
    parser.add_argument(
        "-o",
        "--out",
        default=None,
    ...
    parser.add_argument(
        ...
        choices=["tiles", "image"],
        default="tiles",
        help="`tiles` = download max-res IIIF tile set (default), `image` = single full image per page.",
    )
    parser.add_argument(
        "--workers",
        type=int,
