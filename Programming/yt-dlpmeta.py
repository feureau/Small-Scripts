...
def check_js_runtime():
    """
    Checks for all supported JavaScript runtimes (node, deno, bun, qjs).
    Returns a list of (runtime_name, runtime_path) for all found.
...
# ==========================================
def process_url(url, fetch_full_metadata=False, fetch_comments=False, fetch_sub=False, fetch_heatmap=False, output_format=None, condensed_mode=None, force_single=False, quick_mode=False, sleep_seconds=DEFAULT_SLEEP, js_runtimes=None):
    """
    ...
    ydl_opts = {
        ...
        'remote_components': 'ejs:github', # Enable JS challenge solver (recommended)
    }
    ...
    if js_runtimes:
        # js_runtimes is a list of (name, path) tuples
        # Format for yt-dlp API: dict {name: {'executable': path}}
        ydl_opts['js_runtimes'] = {name: {'executable': path} for name, path in js_runtimes}
    ...
    try:
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            ...
            if is_single_video:
                ...
                handle_channel_or_playlist(info_dict, fetch_full_metadata, fetch_comments, fetch_sub, fetch_heatmap, output_format, condensed_mode, sleep_seconds=sleep_seconds, js_runtimes=js_runtimes)
...
# ==========================================
def handle_channel_or_playlist(info_dict, is_deep_mode, fetch_comments, fetch_sub, fetch_heatmap, output_format, condensed_mode=None, sleep_seconds=DEFAULT_SLEEP, js_runtimes=None):
    """
    ...
    if not is_deep_mode:
        ...
        if fetch_sub or fetch_heatmap:
            ...
            extract_deep_resources(video_entries, fetch_sub, fetch_heatmap, fetch_comments, output_dir=output_dir, condensed_mode=condensed_mode, sleep_seconds=sleep_seconds, js_runtimes=js_runtimes)
...
# ==========================================
def extract_deep_resources(video_entries, fetch_sub, fetch_heatmap, fetch_comments, output_dir=None, condensed_mode=None, sleep_seconds=DEFAULT_SLEEP, js_runtimes=None):
    """
    ...
    if js_runtimes:
        # js_runtimes is a list of (name, path) tuples
        # Format for yt-dlp API: dict {name: {'executable': path}}
        video_ydl_opts['js_runtimes'] = {name: {'executable': path} for name, path in js_runtimes}
...
if __name__ == "__main__":
    ...
    # Check for JS Runtimes (Required by yt-dlp for SABR)
    js_runtimes = check_js_runtime()
        ...
        if args.batch or (os.path.isfile(args.url) and not args.url.startswith(('http://', 'https://'))):
            ...
            try:
                ...
                for index, url in enumerate(urls, 1):
                    ...
                    try:
                        process_url(url, args.full, args.comment, args.sub, args.heatmap, out_fmt, is_condensed, args.single, args.quick, sleep_seconds=args.sleep, js_runtimes=js_runtimes)
                    except Exception as e:
            ...
            process_url(args.url, args.full, args.comment, args.sub, args.heatmap, out_fmt, is_condensed, args.single, args.quick, sleep_seconds=args.sleep, js_runtimes=js_runtimes)
