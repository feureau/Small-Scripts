...
def launch_gui(initial_file_list, initial_file_settings_map, default_qvbr_global, default_hdr_global):
    ...
    main_canvas.create_window((0, 0), window=inner_frame, anchor='nw', tags="inner_frame_window")
    ...
    def _on_mousewheel(event): 
        ...
        while temp_widget is not None:
            if temp_widget == audio_canvas or temp_widget == file_treeview:
                scroll_target = temp_widget
                break
            temp_widget = temp_widget.master
        if not scroll_target: 
            scroll_target = main_canvas
        delta = -1 * (event.delta // 120) if platform.system() == 'Windows' else -1 * event.delta
        if hasattr(scroll_target, 'yview_scroll'):
            scroll_target.yview_scroll(delta, "units")
    root.bind_all("<MouseWheel>", _on_mousewheel)

    def _configure_inner_frame(event): 
        main_canvas.configure(scrollregion=main_canvas.bbox("all"))
        main_canvas.itemconfig("inner_frame_window", width=event.width)
    ...
    def _apply_gui_var_to_selected_files(var_key_name, new_value): 
        ...
        if len(selected_iids) > 1: _update_gui_controls_for_selection(selected_iids) 
    
    def update_metadata_display(selected_file_path_to_display): 
        nonlocal metadata_text_widget
        if not metadata_text_widget: return 
        if not selected_file_path_to_display:
            metadata_text_widget.config(state='normal'); metadata_text_widget.delete("1.0", "end")
            metadata_text_widget.insert("1.0", "No file selected or metadata available."); metadata_text_widget.config(state='disabled')
            return
        ...
        metadata_text_widget.config(state='normal'); metadata_text_widget.delete("1.0", "end")
        metadata_text_widget.insert("1.0", meta_txt); metadata_text_widget.config(state='disabled')
    ...
    def _update_dynamic_audio_track_display(file_path_for_audio): 
        ...
        if not audio_tracks_dynamic_frame : return 
        ...
        # print(f"DEBUG: _update_dynamic_audio_track_display called with: {file_path_for_audio}, is_single: {is_single_file_selected}") 
        if not is_single_file_selected:
            ...
            # print(f"DEBUG: Found {len(tracks_config_list)} audio tracks for {os.path.basename(file_path_for_audio if file_path_for_audio else 'None')}")
        ...
        audio_tracks_dynamic_frame.update_idletasks() 
        if audio_canvas:
            audio_canvas.config(scrollregion=audio_canvas.bbox("all")) 
            audio_canvas.yview_moveto(0) 
    ...
    def _on_no_crop_var_changed(*args):
        ...
        if current_primary_selected_file_path and file_treeview and current_primary_selected_file_path in file_treeview.selection(): 
            _update_crop_fields_for_primary_selection()
    def _on_resolution_var_changed(*args):
        ...
        selected_iids = file_treeview.selection() if file_treeview else []
        if len(selected_iids) == 1 and new_res_val != MIXED_VALUE_DISPLAY: 
             if new_res_val in resolution_map_config: 
    ...
    def _on_denoise_var_changed(*args): 
        ...
        if current_primary_selected_file_path and file_treeview and current_primary_selected_file_path in file_treeview.selection() and current_primary_selected_file_path in file_settings_map: 
    ...
    def on_treeview_select(event): _update_gui_controls_for_selection(file_treeview.selection()) 
    ...
    crop_w_display_var.trace_add("write",lambda n,i,m,v_name="crop_w_display_var":_on_crop_display_var_changed(v_name))
    crop_h_display_var.trace_add("write",lambda n,i,m,v_name="crop_h_display_var":_on_crop_display_var_changed(v_name))
    crop_x_display_var.trace_add("write",lambda n,i,m,v_name="crop_x_display_var":_on_crop_display_var_changed(v_name))
    crop_y_display_var.trace_add("write",lambda n,i,m,v_name="crop_y_display_var":_on_crop_display_var_changed(v_name))
    ...
    audio_tracks_dynamic_frame.config(bg=audio_canvas.cget('background')) 
    ...
    req_w=inner_frame.winfo_reqwidth(); win_w=min(max(req_w+80, 850),screen_width); win_h=int(screen_height*0.90) 
...
def process_video(file_path, settings_bundle): 
    ...
    if output_res_choice_this_file in local_res_map: 
        target_w_map_val, target_h_map_val, _ = local_res_map[output_res_choice_this_file] 
        curr_w,curr_h = crop_w_val,crop_h_val
        if target_h_map_val is not None and curr_h>0: 
