...
v2.6 - Enhanced Subtitle Styling (2025-10-26)
    • Added comprehensive subtitle styling controls:
      - Outline thickness control
      - Shadow depth and blur controls
      - Text opacity/transparency
      - Background box with color and opacity
      - Font weight control (100-900 scale)
      - Secondary outline with separate color
      - Gradient text effects
      - Karaoke/highlight effects
    • Enhanced ASS file generation with advanced styling
    • Updated GUI with advanced styling section
...
# --- New Subtitle Styling Defaults ---
DEFAULT_SUBTITLE_OUTLINE_THICKNESS = "2"
DEFAULT_SUBTITLE_SHADOW_DEPTH = "2"
DEFAULT_SUBTITLE_SHADOW_BLUR = "0"
DEFAULT_SUBTITLE_TEXT_OPACITY = "100"
DEFAULT_SUBTITLE_BACKGROUND_BOX = False
DEFAULT_SUBTITLE_BACKGROUND_COLOR = "#000000"
DEFAULT_SUBTITLE_BACKGROUND_OPACITY = "70"
DEFAULT_SUBTITLE_FONT_WEIGHT = "400"  # 100-900 scale
DEFAULT_SUBTITLE_SECONDARY_OUTLINE = False
DEFAULT_SUBTITLE_SECONDARY_OUTLINE_COLOR = "#000000"
DEFAULT_SUBTITLE_SECONDARY_OUTLINE_THICKNESS = "1"
DEFAULT_SUBTITLE_GRADIENT_ENABLED = False
DEFAULT_SUBTITLE_GRADIENT_COLORS = ["#FFAA00", "#FF0000"]
DEFAULT_SUBTITLE_GRADIENT_ANGLE = "0"
DEFAULT_SUBTITLE_KARAOKE_ENABLED = False
DEFAULT_SUBTITLE_KARAOKE_HIGHLIGHT_COLOR = "#FFFF00"
...
def hex_to_libass_color(hex_color, opacity_percent=100):
    """Convert hex color to libass color format with opacity support"""
    ...
    return f"&H{alpha:02X}{b:02X}{g:02X}{r:02X}"  # ASS uses AABBGGRR format

def parse_srt_time_to_seconds(time_str):
    """Convert SRT time format (HH:MM:SS,mmm) to seconds - FIXED VERSION"""
    try:
        # Handle both comma and period as millisecond separators
        if ',' in time_str:
        ...
        print(f"[ERROR] Failed to parse SRT time '{time_str}': {e}")
        return 0.0

def create_word_by_word_karaoke(text, start_time, end_time, highlight_color, base_color):
    """Create word-by-word karaoke effect with proper timing - FIXED VERSION"""
    # Ensure start_time and end_time are floats with proper error handling
    try:
        start_sec = float(start_time) if not isinstance(start_time, float) else start_time
        end_sec = float(end_time) if not isinstance(end_time, float) else end_time
    except (ValueError, TypeError) as e:
        print(f"[WARN] Invalid timing values: start={start_time}, end={end_time}, error: {e}")
        return f"{{\\2c{base_color}\\1c{highlight_color}\\K100}}{text}"
    
    words = text.strip().split()
    if len(words) <= 1:
        # Single word - highlight immediately with proper color format
        return f"{{\\2c{base_color}\\1c{highlight_color}\\K0}}{text}"
    ...
    # Build karaoke effect with word-by-word timing
    karaoke_parts = []
    
    for i, word in enumerate(words):
        # Calculate cumulative time for this word (in centiseconds)
        cumulative_time = int((i + 1) * word_duration * 100)
        karaoke_parts.append(f"{{\\K{cumulative_time}}}{word}")
    
    # FIXED: Remove trailing '&' from color codes
    result = f"{{\\2c{base_color}\\1c{highlight_color}}}" + ' '.join(karaoke_parts)
    return result

def apply_advanced_subtitle_effects(text, options, start_time=None, end_time=None):
    """Apply advanced styling effects like gradients, karaoke, etc. - FIXED VERSION"""
    # Get colors with proper opacity
    primary_color = hex_to_libass_color(
        options.get('subtitle_primary_color', DEFAULT_SUBTITLE_PRIMARY_COLOR),
        options.get('subtitle_text_opacity', DEFAULT_SUBTITLE_TEXT_OPACITY)
    )
    
    # Start with base styling that ensures visibility
    base_style = f"{{\\1c{primary_color}}}"
    result = text
    
    # Apply gradient if enabled
    if options.get('subtitle_gradient_enabled', DEFAULT_SUBTITLE_GRADIENT_ENABLED):
        color1 = options.get('subtitle_gradient_color1', DEFAULT_SUBTITLE_GRADIENT_COLORS[0])
        color2 = options.get('subtitle_gradient_color2', DEFAULT_SUBTITLE_GRADIENT_COLORS[1])
        angle = options.get('subtitle_gradient_angle', DEFAULT_SUBTITLE_GRADIENT_ANGLE)
        color1_ass = hex_to_libass_color(color1, options.get('subtitle_text_opacity', DEFAULT_SUBTITLE_TEXT_OPACITY))
        color2_ass = hex_to_libass_color(color2, options.get('subtitle_text_opacity', DEFAULT_SUBTITLE_TEXT_OPACITY))
        base_style = f"{{\\1c{color1_ass}\\2c{color2_ass}\\3c{color1_ass}\\4c{color2_ass}\\gradient{angle}}}"

    # FIXED: Karaoke effect - ensure it doesn't make text transparent
    if options.get('subtitle_karaoke_enabled', DEFAULT_SUBTITLE_KARAOKE_ENABLED) and start_time is not None and end_time is not None:
        try:
            # Use full opacity for karaoke to ensure visibility
            highlight_color = hex_to_libass_color(
                options.get('subtitle_karaoke_highlight_color', DEFAULT_SUBTITLE_KARAOKE_HIGHLIGHT_COLOR),
                100  # Force full opacity for highlight
            )
            base_color_karaoke = hex_to_libass_color(
                options.get('subtitle_primary_color', DEFAULT_SUBTITLE_PRIMARY_COLOR),
                100  # Force full opacity for base
            )
            
            # Apply karaoke with timing
            karaoke_text = create_word_by_word_karaoke(text, start_time, end_time, highlight_color, base_color_karaoke)
            result = karaoke_text
        except Exception as e:
            print(f"[WARN] Karaoke effect failed, using basic styling: {e}")
            result = base_style + text
    else:
        # No karaoke - apply base style
        result = base_style + result

    # Apply secondary outline if enabled
    if options.get('subtitle_secondary_outline', DEFAULT_SUBTITLE_SECONDARY_OUTLINE):
        sec_color = hex_to_libass_color(
            options.get('subtitle_secondary_outline_color', DEFAULT_SUBTITLE_SECONDARY_OUTLINE_COLOR)
        )
        sec_thickness = options.get('subtitle_secondary_outline_thickness', DEFAULT_SUBTITLE_SECONDARY_OUTLINE_THICKNESS)
        result = f"{{\\3c{sec_color}\\bord{sec_thickness}}}" + result

    # Apply shadow blur if enabled
    shadow_blur = int(options.get('subtitle_shadow_blur', DEFAULT_SUBTITLE_SHADOW_BLUR))
    if shadow_blur > 0:
        for i in range(shadow_blur):
            offset = i + 1
            result = f"{{\\4a&H80\\shad{offset}}}" + result

    return result

def debug_ass_file_content(ass_path):
    """Debug function to check ASS file styling"""
    try:
        with open(ass_path, 'r', encoding='utf-8') as f:
            content = f.read()
        
        print(f"[ASS DEBUG] File: {ass_path}")
        lines = content.split('\n')
        
        # Find and print style line
        for i, line in enumerate(lines):
            if line.startswith('Style:'):
                print(f"[ASS DEBUG] Style line: {line}")
            
            if line.startswith('Dialogue:'):
                print(f"[ASS DEBUG] Sample dialogue line: {line[:200]}...")
                break
                
        return True
    except Exception as e:
        print(f"[ASS DEBUG] Error reading ASS file: {e}")
        return False

def create_test_ass_file(srt_path, options):
    """Create a simple test ASS file to verify subtitle burning"""
...
Dialogue: 0,0:00:01.00,0:00:10.00,Default,,0,0,0,,This is a test subtitle
Dialogue: 0,0:00:05.00,0:00:15.00,Default,,0,0,0,,Another test subtitle
"""
    
    test_ass_path = srt_path.replace('.srt', '_TEST.ass')
    try:
        with open(test_ass_path, 'w', encoding='utf-8') as f:
            f.write(test_ass_content)
        print(f"[SUBTITLE TEST] Created test ASS file: {test_ass_path}")
        return test_ass_path
    except Exception as e:
        print(f"[SUBTITLE TEST] Failed to create test ASS file: {e}")
        return None

def create_temporary_ass_file(srt_path, options):
    """Create ASS subtitle file in the same folder as the SRT file with timestamp - FIXED VERSION"""
    try:
        with open(srt_path, 'r', encoding='utf-8', errors='replace') as f:
            srt_content = f.read()
        print(f"[SUBTITLE] Read SRT file: {srt_path}, {len(srt_content)} characters")
        
        # Debug: Print first few lines of SRT content
        lines = srt_content.split('\n')
        print(f"[SUBTITLE] SRT file preview (first 10 lines):")
        for i, line in enumerate(lines[:10]):
            print(f"[SUBTITLE] {i+1}: {line}")
    ...
    # Enhanced color handling with opacity - FIXED to ensure non-transparent text
    text_opacity = float(options.get('subtitle_text_opacity', DEFAULT_SUBTITLE_TEXT_OPACITY))
    primary_color = hex_to_libass_color(
        options.get('subtitle_primary_color', DEFAULT_SUBTITLE_PRIMARY_COLOR),
        text_opacity
    )
    outline_color = hex_to_libass_color(
        options.get('subtitle_outline_color', DEFAULT_SUBTITLE_OUTLINE_COLOR)
    )
    shadow_color = hex_to_libass_color(
        options.get('subtitle_shadow_color', DEFAULT_SUBTITLE_SHADOW_COLOR)
    )
    
    # Background box support
    background_box = options.get('subtitle_background_box', DEFAULT_SUBTITLE_BACKGROUND_BOX)
    background_color = hex_to_libass_color(
        options.get('subtitle_background_color', DEFAULT_SUBTITLE_BACKGROUND_COLOR),
        float(options.get('subtitle_background_opacity', DEFAULT_SUBTITLE_BACKGROUND_OPACITY))
    )
    
    # Font weight conversion (100-900 to ASS bold flags)
    font_weight = options.get('subtitle_font_weight', DEFAULT_SUBTITLE_FONT_WEIGHT)
    bold_flag = "-1" if int(font_weight) >= 600 else "0"
    ...
    outline_thickness = options.get('subtitle_outline_thickness', DEFAULT_SUBTITLE_OUTLINE_THICKNESS)
    shadow_depth = options.get('subtitle_shadow_depth', DEFAULT_SUBTITLE_SHADOW_DEPTH)
    
    align_map = {"top": 8, "middle": 5, "bottom": 2}
    alignment = align_map.get(options.get('subtitle_alignment', 'bottom'), 2)
    
    # Build style line with enhanced parameters
    border_style = "4" if background_box else "1"  # 4 = background box
...
[V4+ Styles]
Format: Name, Fontname, Fontsize, PrimaryColour, SecondaryColour, OutlineColour, BackColour, Bold, Italic, Underline, StrikeOut, ScaleX, ScaleY, Spacing, Angle, BorderStyle, Outline, Shadow, Alignment, MarginL, MarginR, MarginV, Encoding
Style: {style_name},{font_name},{font_size},{primary_color},&H000000FF,{outline_color},{background_color},{bold_flag},{italic_flag},0,0,100,100,0,0,{border_style},{outline_thickness},{shadow_depth},{alignment},10,10,{margin_v},1

[Events]
Format: Layer, Start, End, Style, Name, MarginL, MarginR, MarginV, Effect, Text
"""
    
    # Enhanced dialogue processing with advanced effects - FIXED parsing
    dialogue_lines = []
    
    # More robust SRT parsing
    blocks = srt_content.strip().split('\n\n')
    print(f"[SUBTITLE] Found {len(blocks)} blocks in SRT file")
    ...
    for block_index, block in enumerate(blocks):
        try:
            ...
            # Parse block number (ignore if not used)
            ...
            print(f"[SUBTITLE] Block {block_index}: {start_time} --> {end_time}, text: '{text[:50]}...'")
            
            # Convert to ASS format times
            start_ass = start_time.replace(',', '.').replace(' ', '')
            end_ass = end_time.replace(',', '.').replace(' ', '')
            
            # Parse times for debugging
            start_seconds = parse_srt_time_to_seconds(start_time)
            end_seconds = parse_srt_time_to_seconds(end_time)
            print(f"[SUBTITLE] Time in seconds: {start_seconds}s -> {end_seconds}s")
            
            # Apply styling
            styled_text = apply_advanced_subtitle_effects(text, options, start_seconds, end_seconds)
            
            dialogue_lines.append(f"Dialogue: 0,{start_ass},{end_ass},{style_name},,0,0,0,,{styled_text}")
            
        except Exception as e:
            print(f"[ERROR] Error processing subtitle block {block_index}: {e}")
            print(f"[ERROR] Block content: {block}")
            continue

    print(f"[SUBTITLE] Successfully processed {len(dialogue_lines)} dialogue lines")
    full_ass_content = header + "\n".join(dialogue_lines)

    try:
        # Create ASS file in the same folder as the SRT file with timestamp
        srt_dir = os.path.dirname(srt_path)
        srt_basename = os.path.splitext(os.path.basename(srt_path))[0]
        timestamp = time.strftime("%Y%m%d_%H%M%S")
        ass_filename = f"{srt_basename}_{timestamp}.ass"
        ass_file_path = os.path.join(srt_dir, ass_filename)
        
        with open(ass_file_path, 'w', encoding='utf-8') as f:
            f.write(full_ass_content)
        
        print(f"[SUBTITLE] Created ASS file: {ass_file_path}, {len(full_ass_content)} characters")
        
        # Debug the ASS file content
        debug_ass_file_content(ass_file_path)
        
        return ass_file_path
    except Exception as e:
        print(f"[ERROR] Could not create ASS file: {e}")
        return None
...
def get_video_info(file_path):
    ...
    try:
        ...
        # More accurate codec detection - FIXED AV1 detection
        codec_name = data.get("codec_name", "h264")
        codec_tag = data.get("codec_tag_string", "").lower()
        
        # Handle AV1 detection - check both codec_name and codec_tag
...
class VideoProcessorApp:
    def __init__(self, root, initial_files, output_mode):
        ...
        # New subtitle styling variables
        self.subtitle_outline_thickness_var = tk.StringVar(value=DEFAULT_SUBTITLE_OUTLINE_THICKNESS)
        self.subtitle_shadow_depth_var = tk.StringVar(value=DEFAULT_SUBTITLE_SHADOW_DEPTH)
        self.subtitle_shadow_blur_var = tk.StringVar(value=DEFAULT_SUBTITLE_SHADOW_BLUR)
        self.subtitle_text_opacity_var = tk.StringVar(value=DEFAULT_SUBTITLE_TEXT_OPACITY)
        self.subtitle_background_box_var = tk.BooleanVar(value=DEFAULT_SUBTITLE_BACKGROUND_BOX)
        self.subtitle_background_color_var = tk.StringVar(value=DEFAULT_SUBTITLE_BACKGROUND_COLOR)
        self.subtitle_background_opacity_var = tk.StringVar(value=DEFAULT_SUBTITLE_BACKGROUND_OPACITY)
        self.subtitle_font_weight_var = tk.StringVar(value=DEFAULT_SUBTITLE_FONT_WEIGHT)
        self.subtitle_secondary_outline_var = tk.BooleanVar(value=DEFAULT_SUBTITLE_SECONDARY_OUTLINE)
        self.subtitle_secondary_outline_color_var = tk.StringVar(value=DEFAULT_SUBTITLE_SECONDARY_OUTLINE_COLOR)
        self.subtitle_secondary_outline_thickness_var = tk.StringVar(value=DEFAULT_SUBTITLE_SECONDARY_OUTLINE_THICKNESS)
        self.subtitle_gradient_enabled_var = tk.BooleanVar(value=DEFAULT_SUBTITLE_GRADIENT_ENABLED)
        self.subtitle_gradient_color1_var = tk.StringVar(value=DEFAULT_SUBTITLE_GRADIENT_COLORS[0])
        self.subtitle_gradient_color2_var = tk.StringVar(value=DEFAULT_SUBTITLE_GRADIENT_COLORS[1])
        self.subtitle_gradient_angle_var = tk.StringVar(value=DEFAULT_SUBTITLE_GRADIENT_ANGLE)
        self.subtitle_karaoke_enabled_var = tk.BooleanVar(value=DEFAULT_SUBTITLE_KARAOKE_ENABLED)
        self.subtitle_karaoke_highlight_color_var = tk.StringVar(value=DEFAULT_SUBTITLE_KARAOKE_HIGHLIGHT_COLOR)
    ...
    def setup_audio_column(self, parent):
        ...
        # Enhanced Styling Controls Frame
        enhanced_style_frame = tk.LabelFrame(subtitle_group, text="Advanced Styling", padx=10, pady=5)
        enhanced_style_frame.pack(fill=tk.X, pady=5)

        # Outline & Shadow Controls
        outline_shadow_frame = tk.Frame(enhanced_style_frame)
        outline_shadow_frame.pack(fill=tk.X, pady=2)

        tk.Label(outline_shadow_frame, text="Outline:").pack(side=tk.LEFT)
        outline_scale = tk.Scale(outline_shadow_frame, from_=0, to=10, orient=tk.HORIZONTAL, 
                                variable=self.subtitle_outline_thickness_var, showvalue=1, length=80)
        outline_scale.pack(side=tk.LEFT, padx=5)
        outline_scale.bind("<ButtonRelease-1>", self.apply_gui_options_to_selected_jobs_event)

        tk.Label(outline_shadow_frame, text="Shadow:").pack(side=tk.LEFT, padx=(10,0))
        shadow_scale = tk.Scale(outline_shadow_frame, from_=0, to=10, orient=tk.HORIZONTAL,
                               variable=self.subtitle_shadow_depth_var, showvalue=1, length=80)
        shadow_scale.pack(side=tk.LEFT, padx=5)
        shadow_scale.bind("<ButtonRelease-1>", self.apply_gui_options_to_selected_jobs_event)

        tk.Label(outline_shadow_frame, text="Blur:").pack(side=tk.LEFT, padx=(10,0))
        blur_scale = tk.Scale(outline_shadow_frame, from_=0, to=10, orient=tk.HORIZONTAL,
                             variable=self.subtitle_shadow_blur_var, showvalue=1, length=80)
        blur_scale.pack(side=tk.LEFT, padx=5)
        blur_scale.bind("<ButtonRelease-1>", self.apply_gui_options_to_selected_jobs_event)

        # Opacity & Background
        opacity_bg_frame = tk.Frame(enhanced_style_frame)
        opacity_bg_frame.pack(fill=tk.X, pady=2)

        tk.Label(opacity_bg_frame, text="Text Opacity:").pack(side=tk.LEFT)
        opacity_scale = tk.Scale(opacity_bg_frame, from_=0, to=100, orient=tk.HORIZONTAL,
                                variable=self.subtitle_text_opacity_var, showvalue=1, length=100)
        opacity_scale.pack(side=tk.LEFT, padx=5)
        opacity_scale.bind("<ButtonRelease-1>", self.apply_gui_options_to_selected_jobs_event)

        bg_cb = tk.Checkbutton(opacity_bg_frame, text="Background Box", 
                              variable=self.subtitle_background_box_var,
                              command=self.apply_gui_options_to_selected_jobs)
        bg_cb.pack(side=tk.LEFT, padx=(20,5))

        self.bg_color_swatch = tk.Label(opacity_bg_frame, text="    ", 
                                       bg=self.subtitle_background_color_var.get(), 
                                       relief="sunken", width=4)
        self.bg_color_swatch.pack(side=tk.LEFT, padx=2)
        tk.Button(opacity_bg_frame, text="BG Color", 
                 command=lambda: self.choose_color(self.subtitle_background_color_var, self.bg_color_swatch)).pack(side=tk.LEFT, padx=2)

        # Font Weight & Secondary Outline
        weight_outline_frame = tk.Frame(enhanced_style_frame)
        weight_outline_frame.pack(fill=tk.X, pady=2)

        tk.Label(weight_outline_frame, text="Font Weight:").pack(side=tk.LEFT)
        weight_combo = ttk.Combobox(weight_outline_frame, textvariable=self.subtitle_font_weight_var, 
                                   values=["100", "200", "300", "400", "500", "600", "700", "800", "900"], 
                                   width=5, state="readonly")
        weight_combo.pack(side=tk.LEFT, padx=5)
        weight_combo.bind("<<ComboboxSelected>>", self.apply_gui_options_to_selected_jobs_event)

        sec_outline_cb = tk.Checkbutton(weight_outline_frame, text="Secondary Outline",
                                       variable=self.subtitle_secondary_outline_var,
                                       command=self.apply_gui_options_to_selected_jobs)
        sec_outline_cb.pack(side=tk.LEFT, padx=(20,5))

        self.sec_outline_swatch = tk.Label(weight_outline_frame, text="    ",
                                          bg=self.subtitle_secondary_outline_color_var.get(),
                                          relief="sunken", width=4)
        self.sec_outline_swatch.pack(side=tk.LEFT, padx=2)
        tk.Button(weight_outline_frame, text="2nd Color",
                 command=lambda: self.choose_color(self.subtitle_secondary_outline_color_var, self.sec_outline_swatch)).pack(side=tk.LEFT, padx=2)

        # Gradient & Karaoke Effects
        effects_frame = tk.Frame(enhanced_style_frame)
        effects_frame.pack(fill=tk.X, pady=2)

        gradient_cb = tk.Checkbutton(effects_frame, text="Gradient Text",
                                   variable=self.subtitle_gradient_enabled_var,
                                   command=self.apply_gui_options_to_selected_jobs)
        gradient_cb.pack(side=tk.LEFT)

        self.grad_color1_swatch = tk.Label(effects_frame, text="    ",
                                         bg=self.subtitle_gradient_color1_var.get(),
                                         relief="sunken", width=3)
        self.grad_color1_swatch.pack(side=tk.LEFT, padx=2)
        tk.Button(effects_frame, text="G1",
                 command=lambda: self.choose_color(self.subtitle_gradient_color1_var, self.grad_color1_swatch)).pack(side=tk.LEFT, padx=2)

        self.grad_color2_swatch = tk.Label(effects_frame, text="    ",
                                         bg=self.subtitle_gradient_color2_var.get(),
                                         relief="sunken", width=3)
        self.grad_color2_swatch.pack(side=tk.LEFT, padx=2)
        tk.Button(effects_frame, text="G2",
                 command=lambda: self.choose_color(self.subtitle_gradient_color2_var, self.grad_color2_swatch)).pack(side=tk.LEFT, padx=2)

        karaoke_cb = tk.Checkbutton(effects_frame, text="Karaoke Effect",
                                   variable=self.subtitle_karaoke_enabled_var,
                                   command=self.apply_gui_options_to_selected_jobs)
        karaoke_cb.pack(side=tk.LEFT, padx=(20,5))

        self.karaoke_swatch = tk.Label(effects_frame, text="    ",
                                      bg=self.subtitle_karaoke_highlight_color_var.get(),
                                      relief="sunken", width=3)
        self.karaoke_swatch.pack(side=tk.LEFT, padx=2)
        tk.Button(effects_frame, text="K Color",
                 command=lambda: self.choose_color(self.subtitle_karaoke_highlight_color_var, self.karaoke_swatch)).pack(side=tk.LEFT, padx=2)
    ...
    def get_current_gui_options(self):
        ...
        return {
            ...
            "subtitle_outline_thickness": self.subtitle_outline_thickness_var.get(),
            "subtitle_shadow_depth": self.subtitle_shadow_depth_var.get(),
            "subtitle_shadow_blur": self.subtitle_shadow_blur_var.get(),
            "subtitle_text_opacity": self.subtitle_text_opacity_var.get(),
            "subtitle_background_box": self.subtitle_background_box_var.get(),
            "subtitle_background_color": self.subtitle_background_color_var.get(),
            "subtitle_background_opacity": self.subtitle_background_opacity_var.get(),
            "subtitle_font_weight": self.subtitle_font_weight_var.get(),
            "subtitle_secondary_outline": self.subtitle_secondary_outline_var.get(),
            "subtitle_secondary_outline_color": self.subtitle_secondary_outline_color_var.get(),
            "subtitle_secondary_outline_thickness": self.subtitle_secondary_outline_thickness_var.get(),
            "subtitle_gradient_enabled": self.subtitle_gradient_enabled_var.get(),
            "subtitle_gradient_color1": self.subtitle_gradient_color1_var.get(),
            "subtitle_gradient_color2": self.subtitle_gradient_color2_var.get(),
            "subtitle_gradient_angle": self.subtitle_gradient_angle_var.get(),
            "subtitle_karaoke_enabled": self.subtitle_karaoke_enabled_var.get(),
            "subtitle_karaoke_highlight_color": self.subtitle_karaoke_highlight_color_var.get()
        }
    ...
    def update_gui_from_job_options(self, job):
        ...
        # New subtitle styling options
        self.subtitle_outline_thickness_var.set(options.get("subtitle_outline_thickness", DEFAULT_SUBTITLE_OUTLINE_THICKNESS))
        self.subtitle_shadow_depth_var.set(options.get("subtitle_shadow_depth", DEFAULT_SUBTITLE_SHADOW_DEPTH))
        self.subtitle_shadow_blur_var.set(options.get("subtitle_shadow_blur", DEFAULT_SUBTITLE_SHADOW_BLUR))
        self.subtitle_text_opacity_var.set(options.get("subtitle_text_opacity", DEFAULT_SUBTITLE_TEXT_OPACITY))
        self.subtitle_background_box_var.set(options.get("subtitle_background_box", DEFAULT_SUBTITLE_BACKGROUND_BOX))
        self.subtitle_background_color_var.set(options.get("subtitle_background_color", DEFAULT_SUBTITLE_BACKGROUND_COLOR))
        self.subtitle_background_opacity_var.set(options.get("subtitle_background_opacity", DEFAULT_SUBTITLE_BACKGROUND_OPACITY))
        self.subtitle_font_weight_var.set(options.get("subtitle_font_weight", DEFAULT_SUBTITLE_FONT_WEIGHT))
        self.subtitle_secondary_outline_var.set(options.get("subtitle_secondary_outline", DEFAULT_SUBTITLE_SECONDARY_OUTLINE))
        self.subtitle_secondary_outline_color_var.set(options.get("subtitle_secondary_outline_color", DEFAULT_SUBTITLE_SECONDARY_OUTLINE_COLOR))
        self.subtitle_secondary_outline_thickness_var.set(options.get("subtitle_secondary_outline_thickness", DEFAULT_SUBTITLE_SECONDARY_OUTLINE_THICKNESS))
        self.subtitle_gradient_enabled_var.set(options.get("subtitle_gradient_enabled", DEFAULT_SUBTITLE_GRADIENT_ENABLED))
        self.subtitle_gradient_color1_var.set(options.get("subtitle_gradient_color1", DEFAULT_SUBTITLE_GRADIENT_COLORS[0]))
        self.subtitle_gradient_color2_var.set(options.get("subtitle_gradient_color2", DEFAULT_SUBTITLE_GRADIENT_COLORS[1]))
        self.subtitle_gradient_angle_var.set(options.get("subtitle_gradient_angle", DEFAULT_SUBTITLE_GRADIENT_ANGLE))
        self.subtitle_karaoke_enabled_var.set(options.get("subtitle_karaoke_enabled", DEFAULT_SUBTITLE_KARAOKE_ENABLED))
        self.subtitle_karaoke_highlight_color_var.set(options.get("subtitle_karaoke_highlight_color", DEFAULT_SUBTITLE_KARAOKE_HIGHLIGHT_COLOR))
        ...
        self.bg_color_swatch.config(bg=self.subtitle_background_color_var.get())
        self.sec_outline_swatch.config(bg=self.subtitle_secondary_outline_color_var.get())
        self.grad_color1_swatch.config(bg=self.subtitle_gradient_color1_var.get())
        self.grad_color2_swatch.config(bg=self.subtitle_gradient_color2_var.get())
        self.karaoke_swatch.config(bg=self.subtitle_karaoke_highlight_color_var.get())
    ...
    # ---------------------- Audio helper (YouTube-compliant) - FIXED VERSION ----------------------
    def build_audio_segment(self, file_path, options):
        ...
        if audio_mode == "stereo+5.1":
            ...
            if src_channels >= 6:
                ...
                # Downmix to stereo from 5.1
                fc_parts.append(f"[0:a:{src_index}]pan=stereo|FL=0.5*FC+0.707*FL+0.707*BL+0.5*LFE|FR=0.5*FC+0.707*FR+0.707*BR+0.5*LFE[a_stereo]")
                # Keep original 5.1 (may need to remap channels)
                fc_parts.append(f"[0:a:{src_index}]pan=5.1|FL<FL|FR<FR|FC<FC|LFE<LFE|BL<BL|BR<BR[a_5ch]")
            else:
                # Source has fewer channels - upmix to 5.1
                print(f"[AUDIO] Source has {src_channels} channels, upmixing to 5.1")
                if src_channels == 1:
                    # Handle mono source
                    fc_parts.append(f"[0:a:{src_index}]pan=stereo|c0=1.0*c0|c1=1.0*c0[a_stereo]")
                    fc_parts.append(f"[0:a:{src_index}]pan=5.1|FL=1.0*c0|FR=1.0*c0|FC=1.0*c0|LFE=0.1*c0|BL=0.7*c0|BR=0.7*c0[a_5ch]")
                else:
                    # Stereo source
                    fc_parts.append(f"[0:a:{src_index}]anull[a_stereo]")
                    # Enhanced upmix with proper coefficients
                    fc_parts.append(f"[0:a:{src_index}]pan=5.1|FL=0.8*FL+0.2*FR|FR=0.8*FR+0.2*FL|FC=0.707*FL+0.707*FR|LFE=0.118*FL+0.118*FR|BL=0.577*FL|BR=0.577*FR[a_5ch]")
            ...
            # FIXED: Ensure proper sample rate and channel count for both streams
            args.extend([
                "-c:a:0", "aac", "-b:a:0", f"{STEREO_BITRATE_K}k", 
                "-ar:0", str(AUDIO_SAMPLE_RATE), "-ac:0", "2",
                "-c:a:1", "aac", "-b:a:1", f"{SURROUND_BITRATE_K}k", 
                "-ar:1", str(AUDIO_SAMPLE_RATE), "-ac:1", "6",  # FIXED: Ensure 48kHz for both
    ...
    def build_ffmpeg_command_and_run(self, job, orientation, ass_burn=None):
        ...
        # Enhanced debugging for subtitle burning
        print(f"[SUBTITLE DEBUG] Job: {job['display_name']}")
        print(f"[SUBTITLE DEBUG] Burn subtitles option: {options.get('burn_subtitles', DEFAULT_BURN_SUBTITLES)}")
        print(f"[SUBTITLE DEBUG] Subtitle path: {job['subtitle_path']}")
        print(f"[SUBTITLE DEBUG] ASS burn path: {ass_burn}")
    ...
    def construct_ffmpeg_command(self, file_path, output_file, orientation, ass_burn, options):
        ...
        # FIXED: Better decoder fallback logic
        ...
        # FIXED: For AV1 files, always use software decoder if av1_cuvid is not available
        if info["codec_name"] == "av1" and not decoder_available:
            use_cuda_decoder = False
            decoder = "av1"  # Use software AV1 decoder
            print(f"[INFO] Using software AV1 decoder")
        ...
        # FIXED: Simplified filter chain for subtitle burning
        final_filters = []

        # Always start with GPU filters if we have them
        if vf_filters:
            final_filters.extend(vf_filters)

        # If we need to burn subtitles, we must process on CPU
        if ass_burn:
            print(f"[SUBTITLE] Setting up filter chain for subtitle burning")
            
            # If we have GPU filters, download to CPU
            if vf_filters:
                final_filters.append("hwdownload")
                final_filters.append("format=nv12")
            
            # Add any CPU filters (crop, pad, etc.)
            if cpu_filters:
                final_filters.extend(cpu_filters)
            
            # Add subtitle filter - this is critical
            subtitle_path_escaped = ass_burn.replace('\\', '/').replace(':', '\\:')
            subtitle_filter = f"subtitles=filename='{subtitle_path_escaped}'"
            final_filters.append(subtitle_filter)
            print(f"[SUBTITLE] Added subtitle filter: {subtitle_filter}")
            
            # Upload back to GPU for encoding
            if vf_filters:
                final_filters.append("hwupload_cuda")
        else:
            # No subtitle burning - keep existing logic
            if cpu_filters:
                if vf_filters:
                    final_filters.append("hwdownload")
                    final_filters.append("format=nv12")
        ...
        if final_filters:
            ...
            print(f"[FILTERS] Final filter chain: {vf_string}")
    ...
    def start_processing(self):
        ...
        # Optional: Test without karaoke first
        test_without_karaoke = False  # Set to True to test without karaoke
        ...
        for job_index, job in enumerate(self.processing_jobs):
            try:
                ...
                try:
                    # FIXED: Enhanced subtitle burning logic
                    burn_subtitles = job['options'].get("burn_subtitles", DEFAULT_BURN_SUBTITLES)
                    if burn_subtitles and subtitle_path:
                        print(f"[SUBTITLE] Burn subtitles ENABLED for: {os.path.basename(subtitle_path)}")
                        
                        # Optional: Test without karaoke
                        if test_without_karaoke:
                            job['options']['subtitle_karaoke_enabled'] = False
                            print("[TEST] Karaoke disabled for testing")
                        
                        print(f"[SUBTITLE] Creating styled subtitle file from: {subtitle_path}")
                        temp_ass_path = create_temporary_ass_file(subtitle_path, job['options'])
                        if temp_ass_path and os.path.exists(temp_ass_path):
                            print(f"[SUBTITLE] Successfully created ASS file: {temp_ass_path}")
                            print(f"[SUBTITLE] File exists: {os.path.exists(temp_ass_path)}")
                            print(f"[SUBTITLE] File size: {os.path.getsize(temp_ass_path)} bytes")
                        else:
                            print(f"[SUBTITLE ERROR] Failed to create ASS file")
                            temp_ass_path = None
                    else:
                        if burn_subtitles:
                            print(f"[SUBTITLE] Burn subtitles enabled but no subtitle path provided")
                        else:
                            print(f"[SUBTITLE] Burn subtitles disabled")
                    ...
                    # NOTE: We no longer delete the ASS files automatically
                    # They are now saved in the same folder as the SRT files for debugging
                    if temp_ass_path and os.path.exists(temp_ass_path):
                        print(f"[SUBTITLE] ASS file preserved for debugging: {temp_ass_path}")
