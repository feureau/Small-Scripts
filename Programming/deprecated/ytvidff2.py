...
Version History
-------------------------------------------------------------------------------
v6.11 - Implemented CPU `gblur` for High-Quality "Fill" Mode (2025-11-01)
    • FIXED: A fatal error and visual stretching in "Fill" mode caused by
      limitations of a pure-GPU pipeline without specific blur filters.
    • CHANGED: "Fill" mode now uses a robust hybrid GPU/CPU pipeline. The
      background is scaled on the GPU, downloaded to the CPU, then processed
      with a high-quality Gaussian blur (`gblur`) and cropped. This guarantees a
      visually perfect, non-stretched result on all FFmpeg builds.
    • TESTED: All fixes from v6.9 and v6.10 re-verified as compatible with the
      new hybrid "Fill" mode logic. This is the definitive stable version.

v6.10 - Corrected "Fill" Mode Background Stretching (2025-11-01)
    • FIXED: A visual flaw where the blurred background was stretched.
      (Superseded by v6.11's more robust hybrid method).

v6.9 - Final Batch Processing & Metadata Fixes (2025-11-01)
    • FIXED: A bug where unselected jobs would process with incorrect settings.
    • FIXED: An issue with incorrect aspect ratio metadata causing distortion.
...
# --- Fill (Blur Pad) Effect ---
# This now controls the high-quality gblur filter
DEFAULT_BLUR_PAD_INTENSITY = "25" # Controls the sigma (strength) of the gblur filter. Higher is more blurry.
...
# --- Global Capabilities Dictionary ---
FFMPEG_CAPABILITIES = {
    'cuda': False,
    'nvenc': False,
    'has_gpu_compositing_filters': False,
    'checked': False
}
# ----------------------------------------------------------------------------------------------------
env = os.environ.copy()
...
def check_ffmpeg_capabilities():
    if FFMPEG_CAPABILITIES['checked']:
        return

    try:
        hwaccels_cmd = [FFMPEG_CMD, "-hwaccels"]
        hwaccels_result = subprocess.run(hwaccels_cmd, capture_output=True, text=True, timeout=10)
        FFMPEG_CAPABILITIES['cuda'] = "cuda" in hwaccels_result.stdout.lower()

        encoders_cmd = [FFMPEG_CMD, "-encoders"]
        encoders_result = subprocess.run(encoders_cmd, capture_output=True, text=True, timeout=10)
        FFMPEG_CAPABILITIES['nvenc'] = any(x in encoders_result.stdout.lower() for x in ['h264_nvenc', 'hevc_nvenc'])

        filters_cmd = [FFMPEG_CMD, "-filters"]
        filters_result = subprocess.run(filters_cmd, capture_output=True, text=True, timeout=10)
        filters_output = filters_result.stdout.lower()
        
        has_overlay_cuda = 'overlay_cuda' in filters_output
        FFMPEG_CAPABILITIES['has_gpu_compositing_filters'] = has_overlay_cuda

    except Exception as e:
        print(f"[ERROR] Failed to check FFmpeg capabilities: {e}")
    
    FFMPEG_CAPABILITIES['checked'] = True
...
class VideoProcessorApp:
    ...
    def setup_video_column(self, parent):
        ...
        tk.Radiobutton(self.horizontal_rb_frame, text="16:9 (Widescreen)", variable=self.horizontal_aspect_var, value="16:9", command=self._update_selected_jobs).pack(anchor="w")
        tk.Radiobutton(self.horizontal_rb_frame, text="5:4", variable=self.horizontal_aspect_var, value="5:4", command=self._update_selected_jobs).pack(anchor="w")
        tk.Radiobutton(self.horizontal_rb_frame, text="4:3 (Classic TV)", variable=self.horizontal_aspect_var, value="4:3", command=self._update_selected_jobs).pack(anchor="w")
        self.vertical_rb_frame = tk.Frame(self.aspect_ratio_frame)
        tk.Radiobutton(self.vertical_rb_frame, text="9:16 (Shorts/Reels)", variable=self.vertical_aspect_var, value="9:16", command=self._update_selected_jobs).pack(anchor="w")
        tk.Radiobutton(self.vertical_rb_frame, text="4:5 (Instagram Post)", variable=self.vertical_aspect_var, value="4:5", command=self._update_selected_jobs).pack(anchor="w")
        tk.Radiobutton(self.vertical_rb_frame, text="3:4 (Social Post)", variable=self.vertical_aspect_var, value="3:4", command=self._update_selected_jobs).pack(anchor="w")
        ...
        tk.Radiobutton(self.top_video_frame, text="16:9", variable=self.hybrid_top_aspect_var, value="16:9", command=self._update_selected_jobs).pack(side=tk.LEFT)
        tk.Radiobutton(self.top_video_frame, text="4:5", variable=self.hybrid_top_aspect_var, value="4:5", command=self._update_selected_jobs).pack(side=tk.LEFT)
        tk.Radiobutton(self.top_video_frame, text="4:3", variable=self.hybrid_top_aspect_var, value="4:3", command=self._update_selected_jobs).pack(side=tk.LEFT)
        tk.Label(self.top_video_frame, text="Handling:").pack(side=tk.LEFT, padx=(15,5))
        tk.Radiobutton(self.top_video_frame, text="Crop", variable=self.hybrid_top_mode_var, value="crop", command=self._update_selected_jobs).pack(side=tk.LEFT)
        tk.Radiobutton(self.top_video_frame, text="Pad", variable=self.hybrid_top_mode_var, value="pad", command=self._update_selected_jobs).pack(side=tk.LEFT)
        ...
        tk.Radiobutton(self.bottom_video_frame, text="16:9", variable=self.hybrid_bottom_aspect_var, value="16:9", command=self._update_selected_jobs).pack(side=tk.LEFT)
        tk.Radiobutton(self.bottom_video_frame, text="4:5", variable=self.hybrid_bottom_aspect_var, value="4:5", command=self._update_selected_jobs).pack(side=tk.LEFT)
        tk.Radiobutton(self.bottom_video_frame, text="4:3", variable=self.hybrid_bottom_aspect_var, value="4:3", command=self._update_selected_jobs).pack(side=tk.LEFT)
        tk.Label(self.bottom_video_frame, text="Handling:").pack(side=tk.LEFT, padx=(15,5))
        tk.Radiobutton(self.bottom_video_frame, text="Crop", variable=self.hybrid_bottom_mode_var, value="crop", command=self._update_selected_jobs).pack(side=tk.LEFT)
        tk.Radiobutton(self.bottom_video_frame, text="Pad", variable=self.hybrid_bottom_mode_var, value="pad", command=self._update_selected_jobs).pack(side=tk.LEFT)
        ...
        tk.Radiobutton(aspect_handling_frame, text="Fill", variable=self.aspect_mode_var, value="fill", command=self._toggle_upscale_options).pack(side=tk.LEFT)
        ...
        self.rb_hd = tk.Radiobutton(resolution_options_frame, text="HD", variable=self.resolution_var, value="HD", command=self._update_selected_jobs); self.rb_hd.pack(side=tk.LEFT)
        self.rb_4k = tk.Radiobutton(resolution_options_frame, text="4k", variable=self.resolution_var, value="4k", command=self._update_selected_jobs); self.rb_4k.pack(side=tk.LEFT)
        self.rb_8k = tk.Radiobutton(resolution_options_frame, text="8k", variable=self.resolution_var, value="8k", command=self._update_selected_jobs); self.rb_8k.pack(side=tk.LEFT)
        ...
        tk.Radiobutton(upscale_frame, text="Lanczos (Sharp)", variable=self.upscale_algo_var, value="lanczos", command=self._update_selected_jobs).pack(side=tk.LEFT)
        tk.Radiobutton(upscale_frame, text="Bicubic (Balanced)", variable=self.upscale_algo_var, value="bicubic", command=self._update_selected_jobs).pack(side=tk.LEFT)
        tk.Radiobutton(upscale_frame, text="Bilinear (Fast)", variable=self.upscale_algo_var, value="bilinear", command=self._update_selected_jobs).pack(side=tk.LEFT)
        ...
        tk.Radiobutton(output_format_frame, text="SDR", variable=self.output_format_var, value="sdr", command=self._update_selected_jobs).pack(side=tk.LEFT)
        tk.Radiobutton(output_format_frame, text="HDR", variable=self.output_format_var, value="hdr", command=self._update_selected_jobs).pack(side=tk.LEFT)
        ...
        self.lut_entry.bind("<FocusOut>", lambda e: self._update_selected_jobs())
        ...
        self.manual_bitrate_entry.bind("<FocusOut>", lambda e: self._update_selected_jobs())
        tk.Label(bitrate_frame, text="kbps").pack(side=tk.LEFT)
        fruc_frame = tk.Frame(quality_group); fruc_frame.pack(fill=tk.X, pady=(5,0))
        tk.Checkbutton(fruc_frame, text="Enable FRUC", variable=self.fruc_var, command=lambda: [self.toggle_fruc_fps(), self._update_selected_jobs()]).pack(side=tk.LEFT)
        ...
        self.fruc_fps_entry.bind("<FocusOut>", lambda e: self._update_selected_jobs())
    ...
    def setup_audio_column(self, parent):
        ...
        self.font_combo.bind("<<ComboboxSelected>>", lambda e: self._update_selected_jobs())
        self.populate_fonts()
        ...
        font_size_entry.bind("<FocusOut>", lambda e: self._update_selected_jobs())

        style_frame = tk.Frame(general_style_frame); style_frame.pack(fill=tk.X, pady=2)
        tk.Checkbutton(style_frame, text="Bold", variable=self.subtitle_bold_var, command=self._update_selected_jobs).pack(side=tk.LEFT)
        tk.Checkbutton(style_frame, text="Italic", variable=self.subtitle_italic_var, command=self._update_selected_jobs).pack(side=tk.LEFT, padx=15)
        tk.Checkbutton(style_frame, text="Underline", variable=self.subtitle_underline_var, command=self._update_selected_jobs).pack(side=tk.LEFT)
        ...
        tk.Radiobutton(align_frame, text="Top", variable=self.subtitle_alignment_var, value="top", command=self._update_selected_jobs).pack(side=tk.LEFT)
        tk.Radiobutton(align_frame, text="Mid", variable=self.subtitle_alignment_var, value="middle", command=self._update_selected_jobs).pack(side=tk.LEFT, padx=5)
        tk.Radiobutton(align_frame, text="Bot", variable=self.subtitle_alignment_var, value="bottom", command=self._update_selected_jobs).pack(side=tk.LEFT)
        self.seam_align_rb = tk.Radiobutton(align_frame, text="At Seam (Hybrid Only)", variable=self.subtitle_alignment_var, value="seam", command=self._update_selected_jobs)
        ...
        margin_v_entry.bind("<FocusOut>", lambda e: self._update_selected_jobs())
        
        reformat_frame = tk.LabelFrame(subtitle_group, text="Line Formatting", padx=5, pady=5)
        reformat_frame.pack(fill=tk.X, pady=5)
        tk.Checkbutton(reformat_frame, text="Reformat to Single Wrapped Line", variable=self.reformat_subtitles_var, command=self._update_selected_jobs).pack(side=tk.LEFT)
        ...
        wrap_limit_entry.bind("<FocusOut>", lambda e: self._update_selected_jobs())
        ...
        tk.Button(fill_props_frame, text="..", command=lambda: self.choose_color(self.fill_color_var, self.fill_swatch)).grid(row=0, column=2, padx=5)
        fill_alpha_scale = tk.Scale(fill_props_frame, from_=0, to=255, orient=tk.HORIZONTAL, variable=self.fill_alpha_var, showvalue=0, command=lambda val: self._update_selected_jobs())
        ...
        tk.Button(outline_props_frame, text="..", command=lambda: self.choose_color(self.outline_color_var, self.outline_swatch)).grid(row=0, column=2, padx=5)
        outline_alpha_scale = tk.Scale(outline_props_frame, from_=0, to=255, orient=tk.HORIZONTAL, variable=self.outline_alpha_var, showvalue=0, command=lambda val: self._update_selected_jobs())
        ...
        outline_width_entry.bind("<FocusOut>", lambda e: self._update_selected_jobs())
        ...
        tk.Button(shadow_props_frame, text="..", command=lambda: self.choose_color(self.shadow_color_var, self.shadow_swatch)).grid(row=0, column=2, padx=5)
        shadow_alpha_scale = tk.Scale(shadow_props_frame, from_=0, to=255, orient=tk.HORIZONTAL, variable=self.shadow_alpha_var, showvalue=0, command=lambda val: self._update_selected_jobs())
        ...
        shadow_offset_x_entry.bind("<FocusOut>", lambda e: self._update_selected_jobs())
        ...
        shadow_offset_y_entry.bind("<FocusOut>", lambda e: self._update_selected_jobs())
        ...
        shadow_blur_entry.bind("<FocusOut>", lambda e: self._update_selected_jobs())
        ...
        tk.Checkbutton(action_frame, text="Enable Subtitle Burning", variable=self.burn_subtitles_var, command=self._update_selected_jobs).pack(side=tk.LEFT)
        ...
        tk.Radiobutton(self.audio_mode_frame, text="Stereo + 5.1", variable=self.audio_mode_var, value="stereo+5.1", command=self._update_selected_jobs).pack(side=tk.LEFT)
        tk.Radiobutton(self.audio_mode_frame, text="Passthrough", variable=self.audio_mode_var, value="passthrough", command=self._update_selected_jobs).pack(side=tk.LEFT)
        ...
        self.loudness_target_entry.bind("<FocusOut>", lambda e: self._update_selected_jobs())
        ...
        self.loudness_range_entry.bind("<FocusOut>", lambda e: self._update_selected_jobs())
        ...
        self.true_peak_entry.bind("<FocusOut>", lambda e: self._update_selected_jobs())
    ...
    def setup_button_row(self, parent):
        ...
        self.generate_log_checkbox = tk.Checkbutton(parent, text="Generate Log File", variable=self.generate_log_var, command=self._update_selected_jobs); self.generate_log_checkbox.pack(side=tk.LEFT, padx=(10, 0))
    ...
    def _update_selected_jobs(self):
        selected_indices = self.job_listbox.curselection()
        if not selected_indices:
            return
        
        options_to_apply = self.get_current_gui_options()
        
        for index in selected_indices:
            job = self.processing_jobs[index]
            job['options'].update(options_to_apply)
            
        debug_print(f"Applied full GUI options to {len(selected_indices)} jobs.")
    ...
    def build_ffmpeg_command_and_run(self, job, orientation):
        ...
        try:
            _, temp_options = self.construct_ffmpeg_command(job, output_file, orientation)
            if temp_options.get("burn_subtitles") and job.get('subtitle_path'):
                ...
                if subtitle_source_file:
                    ass_burn_path = create_temporary_ass_file(subtitle_source_file, temp_options)
                    if not ass_burn_path:
                        raise VideoProcessingError(f"Failed to create styled ASS file from {subtitle_source_file}")
            
            cmd, _ = self.construct_ffmpeg_command(job, output_file, orientation, ass_burn_path)
    ...
    def construct_ffmpeg_command(self, job, output_file, orientation, ass_burn_path=None):
        ...
        if orientation != "original":
            ...
            if aspect_mode == 'fill':
                debug_print("Using hybrid GPU/CPU 'Fill' mode with gblur.")
                blur_intensity = options.get('blur_pad_intensity', DEFAULT_BLUR_PAD_INTENSITY)
                # FIXED: Explicitly set the pixel format for both overlay_cuda inputs to nv12.
                # The background stream (after CPU processing and re-upload) and the foreground stream
                # can have conflicting pixel formats (e.g., yuv420p vs nv12), causing an error.
                # Forcing both to nv12 ensures compatibility.
                fill_fc_parts = [
                    f"[0:v]split=2[fg_gpu][bg_gpu]",
                    f"[bg_gpu]scale_cuda=w={target_w}:h={target_h}:force_original_aspect_ratio=increase:interp_algo={upscale_algo},hwdownload,format=nv12[bg_for_cpu]",
                    f"[bg_for_cpu]gblur=sigma={blur_intensity},crop={target_w}:{target_h},hwupload_cuda,format=nv12[bg_blurred_gpu]",
                    f"[fg_gpu]scale_cuda=w={target_w}:h={target_h}:force_original_aspect_ratio=decrease:interp_algo={upscale_algo},format=nv12[fg_sharp_gpu]",
                    f"[bg_blurred_gpu][fg_sharp_gpu]overlay_cuda=(W-w)/2:(H-h)/2[composition_gpu]"
                ]
                video_out_tag = "[composition_gpu]"
                filter_complex_parts.extend(fill_fc_parts)
            else:
                scale_base = f"scale_cuda=w={target_w}:h={target_h}:interp_algo={upscale_algo}"
                if aspect_mode == 'pad':
                    filter_complex_parts.append(f"[0:v]{scale_base}:force_original_aspect_ratio=decrease,pad_cuda=w={target_w}:h={target_h}:x=-1:y=-1:color=black[v_out]")
                elif aspect_mode == 'crop':
                    filter_complex_parts.append(f"[0:v]{scale_base}:force_original_aspect_ratio=increase,hwdownload,format=nv12,crop={target_w}:{target_h},hwupload_cuda[v_out]")
                else: # Stretch
                    filter_complex_parts.append(f"[0:v]{scale_base}[v_out]")
                video_out_tag = "[v_out]"

            cpu_filters_needed = (info["is_hdr"] and not is_hdr_output) or options.get("fruc") or ass_burn_path
            if cpu_filters_needed:
                cpu_pix_fmt = "p010le" if info["bit_depth"] == 10 else "nv12"
                cpu_chain = []
                filter_complex_parts.append(f"{video_out_tag}hwdownload,format={cpu_pix_fmt}[cpu_in]")
                ...
                filter_complex_parts.append(f"[cpu_in]{','.join(cpu_chain)}[v_out_final]")
                video_out_tag = "[v_out_final]"
            
            # FIXED: Add the -aspect flag to correct output metadata.
            cmd.extend(["-aspect", aspect_str])
    ...
    def start_processing(self):
        # FIXED: Apply the current GUI state to ALL jobs before starting.
        if self.processing_jobs:
            print("[INFO] Synchronizing all jobs to current GUI settings...")
            current_options = self.get_current_gui_options()
            for job in self.processing_jobs:
                job['options'].update(current_options)
        
        if not self.processing_jobs: messagebox.showwarning("No Jobs", "Please add files."); return

        for job in self.processing_jobs:
            if job['options'].get('aspect_mode') == 'fill' and not FFMPEG_CAPABILITIES['has_gpu_compositing_filters']:
                messagebox.showerror("Missing FFmpeg Filter", 
                                     "The 'Fill' aspect mode requires FFmpeg to be compiled with the 'overlay_cuda' filter.\n\n"
                                     "Your current build is missing this feature.\n\n"
                                     "Please install a full CUDA-enabled FFmpeg build (e.g., from gyan.dev) to use this feature.")
                return
...
if __name__ == "__main__":
    ...
    try:
        ...
        messagebox.showerror("FFmpeg Not Found", "ffmpeg.exe was not found in your system's PATH.\n\nPlease install FFmpeg and ensure its location is added to your PATH environment variable.")
        sys.exit(1)

    if not FFMPEG_CAPABILITIES['cuda'] or not FFMPEG_CAPABILITIES['nvenc']:
        error_msg = "A critical component for hardware acceleration is missing:\n"
        if not FFMPEG_CAPABILITIES['cuda']: error_msg += "\n• CUDA support in FFmpeg."
        if not FFMPEG_CAPABILITIES['nvenc']: error_msg += "\n• NVENC encoders (h264_nvenc, hevc_nvenc)."
        error_msg += "\n\nPlease install a full FFmpeg build with CUDA/NVENC support enabled."
