...
ffmpeg -hwaccel cuda -i %%A -c:v av1_nvenc -cq:v 16  -b_ref_mode:v middle -c:a copy -map 0 -preset p4 -tune hq -lookahead_level auto -profile:v main10 -pix_fmt p010le -color_primaries bt2020 -colorspace bt2020nc -color_trc smpte2084 -spatial-aq 1 -temporal-aq 1 -vf "scale=1080:1350:force_original_aspect_ratio=1:flags=lanczos,pad=1080:1350:(ow-iw)/2:(oh-ih)/2" %%A_temp_HDR_x2.mkv
