...
FOR %%A IN (%*) DO (
    ...
    ffmpeg -y -init_hw_device cuda=gpu:0 -filter_hw_device gpu -hwaccel cuvid -hwaccel_output_format cuda -i %%A -c:v hevc_nvenc -cq:v 16 -c:a copy -preset p1 -tune ull -lookahead_level auto -rc-lookahead 58 -profile:v main10 -b_ref_mode:v middle -spatial-aq 1 -temporal-aq 1 -vf "scale_cuda='iw*2:ih*2:force_original_aspect_ratio=decrease:force_divisible_by=4:interp_algo=lanczos'" %%A_temp_HDR_x2.mkv 
