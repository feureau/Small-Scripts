...
FOR %%A IN (%*) DO (
    ...
    ffmpeg -y -hwaccel cuda -i %%A -c:v hevc_nvenc -cq:v 16  -b_ref_mode 0 -c:a copy -map 0  -preset p1 -tune ull -lookahead_level auto -profile:v main10 -spatial-aq 1 -vf "scale=7680:7680:force_original_aspect_ratio=decrease:flags=lanczos" -movflags use_metadata_tags -map_metadata 0 -pix_fmt p010le %%A.Horz.8K.HDR.mov
