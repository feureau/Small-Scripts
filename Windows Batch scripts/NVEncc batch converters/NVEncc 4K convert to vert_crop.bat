...
ffmpeg -hwaccel cuda -i %%A_temp_HDR_4K.mkv -c:v av1_nvenc -g 0 -bf 0 -cq:v 16 -b_ref_mode:v middle -c:a copy -map 0 -preset p4 -tune hq -lookahead_level auto -profile:v main10 -pix_fmt p010le -color_primaries bt2020 -colorspace bt2020nc -color_trc smpte2084 -spatial-aq 1 -temporal-aq 1 -vf "scale_cuda=2160:2700:force_original_aspect_ratio=2:interp_algo=lanczos,crop=2160:2700" %%A_temp_HDR_4K_vert_crop.mkv
