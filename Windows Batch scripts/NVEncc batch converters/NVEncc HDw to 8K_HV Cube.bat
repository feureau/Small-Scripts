...
start /b /low /wait NVEncC64 --avhw --codec hevc --profile high --qvbr 0 --preset p4 --output-depth 10 --multipass 2pass-full --lookahead 32 --nonrefp --aq --aq-temporal --aq-strength 0 --transfer auto --audio-copy --chapter-copy --key-on-chapter --sub-copy --metadata copy --vpp-resize algo=ngx-vsr,vsr-quality=1 --output-res 4320x4320,preserve_aspect_ratio=increase -i %%A_temp_HDR_4K.mkv -o %%A_HDR_8K_Horz.mkv 

start /b /low /wait NVEncC64 --avhw --codec hevc --profile high --qvbr 0 --preset p4 --output-depth 10 --multipass 2pass-full --lookahead 32 --nonrefp --aq --aq-temporal --aq-strength 0 --transfer auto --audio-copy --chapter-copy --key-on-chapter --sub-copy --metadata copy --crop 1056,0,1056,0 --vpp-resize algo=ngx-vsr,vsr-quality=1 --output-res 4320x4320,preserve_aspect_ratio=increase -i %%A_temp_HDR_4K.mkv -o %%A_HDR_8K_Vert.mkv 
...
start /b /low /wait mkvmerge.exe -o %%A_HDR_8K_Vert_CUBE.mkv --colour-matrix 0:9 --colour-range 0:1 --colour-transfer-characteristics 0:16 --colour-primaries 0:9 --max-content-light 0:1000 --max-frame-light 0:300 --max-luminance 0:1000 --min-luminance 0:0.01 --chromaticity-coordinates 0:0.68,0.32,0.265,0.690,0.15,0.06 --white-colour-coordinates 0:0.3127,0.3290 --attachment-mime-type application/x-cube --attach-file "C:\ProgramData\Blackmagic Design\DaVinci Resolve\Support\LUT\Colorspace LUTS\5-NBCU_PQ2SDR_DL_RESOLVE17-VRT_v1.2.cube" %%A_HDR_8K_Vert.mkv 

mkvinfo.exe %%A_HDR_8K_Horz_CUBE.mkv

mkvinfo.exe %%A_HDR_8K_Vert_CUBE.mkv

start /b /low /wait mkvmerge.exe -o %%A_HDR_8K_Vert_CUBE-%%c.mkv --split chapters:all %%A_HDR_8K_Vert_CUBE.mkv
