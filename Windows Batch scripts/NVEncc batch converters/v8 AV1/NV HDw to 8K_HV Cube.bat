...
start /b /belownormal /wait NVEncC64 --avhw --codec av1 --tier 1 --profile main --cqp 12 --preset p1 --output-depth 10 --multipass 2pass-full --lookahead 32 --nonrefp --aq --aq-temporal --aq-strength 0 --transfer auto --audio-copy --chapter-copy --key-on-chapter --sub-copy --metadata copy --vpp-nvvfx-artifact-reduction mode=0 --vpp-resize algo=nvvfx-superres,superres-mode=0 --output-res 2160x2160,preserve_aspect_ratio=increase -i %%A -o %%A_temp_HDR_4K.mkv 
...
start /b /belownormal /wait NVEncC64 --avhw --codec av1 --tier 1 --profile main --cqp 12 --preset p1 --output-depth 10 --transfer auto --audio-copy --chapter-copy --key-on-chapter --sub-copy --metadata copy --crop 1056,0,1056,0 --vpp-resize algo=ngx-vsr,vsr-quality=1 --output-res 4320x4320,preserve_aspect_ratio=increase -i %%A_temp_HDR_4K.mkv -o %%A_HDR_8K_Vert.mkv 
